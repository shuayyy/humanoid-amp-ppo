...
class TrainConfig:
  env: G1LocomotionManagerBasedRlEnvCfg | G1DualarmManagerBasedRlEnvCfg
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  if isinstance(cfg.env, G1LocomotionManagerBasedRlEnvCfg):
    env = G1LocomotionManagerBasedRlEnv(
      cfg=cfg.env,
      device=device,
      render_mode="rgb_array" if cfg.video else None,
    )
  elif isinstance(cfg.env, G1DualarmManagerBasedRlEnvCfg):
  ...
  if cfg.video and rank == 0:
    checkpoint_video_interval = cfg.agent.save_interval * cfg.agent.num_steps_per_env
    env = VideoRecorder(
      env,
      video_folder=Path(log_dir) / "videos" / "train",
      step_trigger=lambda step: step % checkpoint_video_interval == 0,
      video_length=cfg.video_length,
      disable_logger=True,
    )
    print("[INFO] Recording videos during training at checkpoint intervals.")
