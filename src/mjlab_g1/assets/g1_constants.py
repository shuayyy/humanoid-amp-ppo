...
G1_XML: Path = Path(
  os.path.join(os.path.dirname(__file__), "g1", "g1_realsense.xml")
)
...
# MjSpec is the editable MuJoCo model loaded from XML before it is compiled into simulation.
...
G1_ACTUATOR_WRIST = BuiltinPositionActuatorCfg(
  target_names_expr=(".*_wrist_.*_joint",),
...
G1_INIT_KEYFRAME = EntityCfg.InitialStateCfg(
  pos=(-0.03, 0.1, 0.78),
  joint_pos={".*": 0.0},
  joint_vel={".*": 0.0},
)
