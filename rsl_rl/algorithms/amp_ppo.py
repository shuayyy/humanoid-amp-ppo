...
class AMP_PPO:
    ...
    def __init__(
        ...
        clip_param=0.2,
        gamma=0.99,
        lam=0.95, #gae: generalized advantage estimation, lambda parameter (It decides how many future rewards are used to compute advantage.)
        value_loss_coef=1.0,
        entropy_coef=0.01,
        learning_rate=0.001,
        max_grad_norm=1.0,
    ...
    def update(self):  # noqa: C901
        ...
        amp_expert_generator = self.amp_data.feed_forward_generator_29dof_multi(
            self.num_learning_epochs * self.num_mini_batches,
            self.storage.num_envs * self.storage.num_transitions_per_env // self.num_mini_batches,
        )
